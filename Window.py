...
def GetSetting(root, SettingScreen:list, MainScreen:callable, unitsIndex:int, fontHEAD, FONT, red:str, blue:str, units:bool):
    ...
    for frame in SettingScreen:
        frame.grid_forget()
    currHead,currFrame,highHead,highFrame = MainScreen(root,fontHEAD,FONT,red,blue,lenCurr,lenHigh)
...
def SettingScreenCreate(root, START:callable, FONT, bgColor:str, unitsIndex:int, units:bool):
    ...
    refreshRateScale = ttk.Scale(root, from_=50, to=500, orient=tk.HORIZONTAL, length=200, variable=varRef)
    refreshRateScale.grid(row=4, column=1, columnspan=2, padx=padding, pady=padding)
    refreshRateScale.bind("<Motion>", getRefreshRate)
    SettingScreen.append(refreshRateScale)

    varRem = tk.StringVar(value=defaultRemove)
    removeRateScale = ttk.Scale(root, from_=10, to=360, orient=tk.HORIZONTAL, length=200, variable=varRem)
    removeRateScale.grid(row=6, column=1, columnspan=2, padx=padding, pady=padding)
    removeRateScale.bind("<Motion>", getRemoveRate)
    SettingScreen.append(removeRateScale)

    refreshRateNum = tk.Label(root, bg=bgColor, font=FONT, text=f"{defaultRefresh} ms")
    refreshRateNum.grid(row=5, column=1, columnspan=2, padx=padding, pady=padding)
    SettingScreen.append(refreshRateNum)

    removeRateNum = tk.Label(root, bg=bgColor, font=FONT, text=f'{defaultRemove} min')
    removeRateNum.grid(row=7, column=1, columnspan=2, padx=padding, pady=padding)
    SettingScreen.append(removeRateNum)

    # LABELS
    currentRowsTxt = tk.Label(root, bg=bgColor, font=FONT, text="Number of current processes:", justify='right')
    currentRowsTxt.grid(row=2, column=0, padx=padding, pady=padding, sticky="ne")
    SettingScreen.append(currentRowsTxt)

    highestRowsTxt = tk.Label(root, bg=bgColor, font=FONT, text="Number of highest processes:", justify='right')
    highestRowsTxt.grid(row=3, column=0, padx=padding, pady=padding, sticky="ne")
    SettingScreen.append(highestRowsTxt)

    refreshRateTxt = tk.Label(root, bg=bgColor, font=FONT, text='Refresh Rate:\nTime for updating processes', justify='right')
    refreshRateTxt.grid(row=4, rowspan=2, column=0, padx=padding, pady=padding, sticky='ne')
    SettingScreen.append(refreshRateTxt)

    removeRateTxt = tk.Label(root, bg=bgColor, font=FONT, text='Time for keeping processes\nwith highest resource usage', justify='right')
    removeRateTxt.grid(row=6, rowspan=2, column=0, padx=padding, pady=padding, sticky="ne")
    SettingScreen.append(removeRateTxt)

    if units: # Only for Memory
        totalRAMtxt = tk.Label(root, bg=bgColor, font=FONT, text="Amount of RAM in PC", justify='right')
        totalRAMtxt.grid(row=0, column=0, padx=padding, pady=padding, sticky="ne")
        SettingScreen.append(totalRAMtxt)

        optionsUnitTxt = tk.Label(root, bg=bgColor, font=FONT, text="Choose units", justify='right')
        optionsUnitTxt.grid(row=1, column=0, padx=padding, pady=padding, sticky="ne")
        SettingScreen.append(optionsUnitTxt)

        totalRAMunit = tk.Label(root, bg=bgColor, font=FONT, text="GB")
        totalRAMunit.grid(row=0, column=2, padx=padding, pady=padding, sticky='w')
        ...
        totalRAM = ttk.Combobox(root, width=6, font=FONT, values=list(2**i for i in range(2,8)))
        totalRAM.grid(row=0, column=1, padx=padding, pady=padding)
        totalRAM.set(32)
        SettingScreen.append(totalRAM)

    else: # Only for CPU
        totalCoresTxt = tk.Label(root, bg=bgColor, font=FONT, text='Amount of Threads in CPU:\nAMD: Threads = Cores count x 2', justify='right')
        totalCoresTxt.grid(row=0, column=0, padx=padding, pady=padding, sticky="ne")
        SettingScreen.append(totalCoresTxt)

        totalCores = ttk.Combobox(root, width=6, font=FONT, values=list(2**i for i in range(1,7)))
...
def MainScreen(root, fontHEAD, FONT, red:str, blue:str, lenCurr:int, lenHigh:int):
    ...
    currFrame = tk.Label(root, anchor='ne', font=FONT, text="", justify="right", background=red, width=int(WIDTH/fontWidthLength), height=lenCurr, foreground=txtColor)
    ...
    highHead
    highFrame = tk.Label(root, anchor='ne', font=FONT, text="", justify="right", background=blue, width=int(WIDTH/fontWidthLength), height=lenHigh, foreground=txtColor)
